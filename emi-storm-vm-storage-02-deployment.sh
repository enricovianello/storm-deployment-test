...
set_users() {
  # storm user
    ...
    if [ $storm_uid -ne 991 ]; then
  		# set uid=991 and gid=991
			execute "usermod --uid 991 storm"
			execute "groupmod -g 991 storm"
			execute "usermod --gid 991 storm"
		fi
	else
		#not exists: create
		echo "user storm does not exist"
		#not exists: create with uid=991 and gid=991
  	execute "groupadd -g 991 storm"
		execute "useradd -u 991 -g 991 storm"
...
add_tape_storage_area() {
	local output_file=$1
	
	echo "configuring TAPE storage area into $output_file..."
	
	echo "adding 'tape' storage area to STORM_STORAGEAREA_LIST..."
	local riga=$(grep STORM_STORAGEAREA_LIST $output_file | grep -v "#")
	echo "riga=$riga"
	local dimriga=${#riga}
	echo "dimriga=$dimriga"
	local newriga="${riga:0:dimriga-1} tape\""
	echo "newriga=$newriga"
	local linenum=$(awk '/STORM_STORAGEAREA_LIST/{print NR}' $output_file)
	echo "linenum=$linenum"
	
	echo "delete line $linenum"
	awk -v n=$linenum 'NR == n {next} {print}' "$output_file.tmp"
	...
	echo "STORM_TAPE_ACCESSPOINT=/stormtest" >> $output_file
	echo "STORM_TAPE_FSDRIVER=\"gpfs\"" >> $output_file
	echo "STORM_TAPE_FSTYPE=\"gpfs\"" >> $output_file
	echo "STORM_TAPE_ACLMODE=\"aot\"" >> $output_file
	echo "STORM_TAPE_NEARLINE_SIZE=2441" >> $output_file
	echo "STORM_TAPE_ONLINE_SIZE=1024" >> $output_file
	echo "STORM_TAPE_QUOTA=\"true\"" >> $output_file
	echo "STORM_TAPE_QUOTA_DEVICE=\"gemss_test\"" >> $output_file
	echo "STORM_TAPE_QUOTA_FILESET=\"stormtest\"" >> $output_file
	echo "STORM_TAPE_RFIO_SUPPORT=\"false\"" >> $output_file
	echo "STORM_TAPE_ROOT=\"/gpfs/gemss_test/test/data1/stormtest\"" >> $output_file
	echo "STORM_TAPE_ROOT_SUPPORT=\"false\"" >> $output_file
	echo "STORM_TAPE_SPCDRIVER=\"gpfs\"" >> $output_file
	echo "STORM_TAPE_STORAGECLASS=\"T1D0\"" >> $output_file
	echo "STORM_TAPE_ANONYMOUS_HTTP_READ=true" >> $output_file
	
	echo "TAPE storage area added to $output_file"
}
...
echo "StoRM 1.11 Deployment started!"
